...
function compress(func, rootDir, type) {
    return __awaiter(this, void 0, void 0, function* () {
        const deps = yield buildDeps(func, rootDir, type);
        const zip = new JSZip();
        if (deps) {
            debug('load deps zip');
            zip.loadAsync(deps, {
        ...
        for (let list of results) {
            for (let filename of list) {
                ...
                if (stats.isFile()) {
                    ...
                    zip.file(filename, yield fs.readlink(filepath), {
...
function zipFolder(zipArchiver, folder, folders, funignore, codeUri, prefix = '') {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const dirItems = yield fs.readdir(dir);
        ...
        return (yield Promise.all(dirItems.map((f) => __awaiter(this, void 0, void 0, function* () {
            const fPath = path.join(dir, f);
            debug('before zip: lstat fPath: %s, absolute fPath is %s', fPath, path.resolve(fPath));
            let s;
            try {
                s = yield fs.lstat(fPath);
            }
            catch (error) {
            ...
            if (s.size === 1067) {
                const content = yield readLines(fPath);
            ...
            if (s.isFile() || s.isSymbolicLink()) {
                ...
                return yield zipFolder(zipArchiver, f, folders.slice(), funignore, codeUri, prefix);
            }
...
function packFromJson(files) {
    return __awaiter(this, void 0, void 0, function* () {
        const zip = new JSZip();
        _.forEach(files, (content, path) => {
            zip.file(path, content);
        });
        return yield generateAsync(zip);
    });
}
function packTo(file, funignore, targetPath, prefix = '', zlibOptions = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (stats.isFile()) {
            ...
            count = yield zipFolder(zipArchiver, file, [], funignore, file, prefix);
        }
        else {
...
function pack(file, funignore) {
    return __awaiter(this, void 0, void 0, function* () {
        const { randomDir, zipPath } = yield generateRandomZipPath();
        const { count, compressedSize } = yield packTo(file, funignore, zipPath);
