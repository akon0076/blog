...
function registerApis(tpl, app, serverPort, debugPort, debugIde, baseDir, debuggerPath, debugArgs, nasBaseDir, tplPath) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield httpSupport.registerApis(app, serverPort, functions, debugPort, debugIde, baseDir, debuggerPath, debugArgs, nasBaseDir, tplPath);
    });
}
...
function start(options, invokeName = '') {
    return __awaiter(this, void 0, void 0, function* () {
        let tpl, tplPath;
        ...
        if (options.debugPort && invokeName) {
            ({
                isDotnetcore,
                tpl, tplPath,
                serviceName,
                functionName, functionRes
            } = yield mergeTplWithoutBuildYml(options.template, invokeName));
        }
        if (isDotnetcore) {
            console.log(yellow(`using template: ${path.relative(process.cwd(), tplPath)}`));
            const baseDir = path.resolve(path.dirname(tplPath));
            yield transformDotnetCodeUri(baseDir, serviceName, functionName, functionRes);
        }
        else {
            ({ tpl, tplPath } = yield generateMergedTpl(options.template, true));
        }
        ...
        if (!_.isEmpty(routes)) {
            ...
            if (functionName) {
                ...
                if (httpTrigger.length === 0) {
                    ...
                    if (!functionRes) {
                        throw new Error(`Error: function ${invokeName} not found in ${tplPath}`);
                    }
                ...
                yield httpSupport.registerSingleHttpTrigger(app, router, serverPort, httpTrigger[0], debugPort, debugIde, baseDir, true, debuggerPath, debugArgs, nasBaseDir, tplPath);
                startExpress(app);
                return;
            }
        }
        if (debugPort) {
            ...
            if (httpTriggers.length > 1) {
                const { path, serviceName, functionName } = yield promptForDebugaHttptriggers(httpTriggers);
            ...
            yield httpSupport.registerSingleHttpTrigger(app, router, serverPort, debugFunction[0], debugPort, debugIde, baseDir, true, debuggerPath, debugArgs, nasBaseDir, tplPath);
        }
        else {
            yield httpSupport.registerHttpTriggers(app, router, serverPort, httpTriggers, debugPort, debugIde, baseDir, debuggerPath, debugArgs, nasBaseDir, tplPath);
        }
        if (_.isEmpty(routes)) {
            yield registerApis(tpl, app, serverPort, debugPort, debugIde, baseDir, debuggerPath, debugArgs, nasBaseDir, tplPath);
        }
        startExpress(app);
