...
class LocalRuntime {
    ...
    invoke(config, event, debugInfo = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const functionId = this._buildIdString(config.region, config.accountId, config.serviceName, config.functionName);
            debug(`Build function id ${functionId}`);
            const codeDir = yield this._getCodeDir(config.codeAbsPath);
            const archive = yield LocalRuntime._getCodeArchive(codeDir);
            debug(`Code archive tarred at ${archive}.`);
            // Constructing a path matches the target OS.
            const workingDir = `${WORKING_DIR_PREFIX}/${identifier.serviceName}/${identifier.functionName}`;
            debug(`Installing code tar archive into ${workingDir} on edge...`);
            yield this.container.copy(archive, workingDir);
            const checksum = yield this._buildChecksum(archive);
            debug(`Code checksum is ${checksum}.`);
            yield this.client.deploy({
                debugPort,
                runtime,
                functionId,
...
`;
     ...
     * @private
     */
    _getCodeDir(codePath) {
        return __awaiter(this, void 0, void 0, function* () {
            const stats = yield fs.stat(codePath);
            if (stats.isFile()) {
     ...
     * @private
     */
    static _getCodeArchive(codeDir) {
        return __awaiter(this, void 0, void 0, function* () {
            const stats = yield fs.stat(codeDir);
            if (stats.isDirectory()) {
                const os = require('os');
                const path = require('path');
                const basename = path.basename(codeDir);
                const tmpdir = yield fs.mkdtemp(path.join(os.tmpdir(), '/'));
