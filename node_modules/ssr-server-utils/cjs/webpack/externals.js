...
function nodeExternals(options) {
    ...
    function isNotBinary(x) {
        return !external_utils_1.contains(binaryDirs, x);
    }
    // create the node modules list
    let nodeModules = [];
    if (modulesFromFile) {
        nodeModules = external_utils_1.readFromPackageJson(options.modulesFromFile);
    }
    else {
        if (Array.isArray(modulesDir)) {
            modulesDir.map(str => {
                nodeModules = nodeModules.concat(external_utils_1.readDir(str).filter(isNotBinary));
            });
        }
        else {
            nodeModules = external_utils_1.readDir(modulesDir).filter(isNotBinary);
        }
    }
    // return an externals function
    return function (context, request, callback) {
        const moduleName = getModuleName(request, includeAbsolutePaths);
        if (external_utils_1.contains(nodeModules, moduleName) && !external_utils_1.containsPattern(whitelist, request)) {
