...
const getDevProxyMiddlewaresArr = async (options) => {
    const { fePort, proxy, isDev, https } = loadConfig_1.loadConfig();
    ...
    function registerProxy(proxy) {
        for (const path in proxy) {
            ...
            const middleware = express ? http_proxy_middleware_1.createProxyMiddleware(path, options) : koaConnect(http_proxy_middleware_1.createProxyMiddleware(path, options));
